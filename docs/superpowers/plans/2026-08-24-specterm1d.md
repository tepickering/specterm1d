...
**Tech Stack:** Python >= 3.11, numpy, scipy, matplotlib (Agg), astropy, specutils, Pillow; optional pypeit and libsixel-python. pytest for tests.
...
- **Python >= 3.11.** Development uses the existing `pypeit3.14` conda env, which has `specutils 2.4.0` and a pypeit dev build. Run tests with that env's interpreter: `/Users/tim/conda/envs/pypeit3.14/bin/python -m pytest`.
- **Required dependencies:** numpy **>= 2.0** (`np.trapezoid`; `np.trapz` no longer exists in 2.x), scipy, matplotlib, astropy, specutils. Pillow arrives via matplotlib.
...
- **Wavelength invariant:** `wave` is float64 and strictly ascending. Non-positive and non-finite wavelengths are forced masked. Real data violates this: UVES `spec1d` `OPT_WAVE` arrays contain zeros in unextracted pixels.
...
dependencies = [
    # numpy 2.0+: fitting.py uses np.trapezoid, and np.trapz is gone in 2.x
    "numpy>=2.0",
    "scipy>=1.10",
    "matplotlib>=3.7",
    "astropy>=6.0",
...
def test_build_spec_masks_nonpositive_wavelengths():
...
def build_spec(
    ...
    *,
    ...
    non-positive and non-finite wavelengths forced masked. Real pypeit
    echelle files rely on this - UVES ``OPT_WAVE`` arrays are zero-padded in
    unextracted pixels.
...
- UVES `spec1d` `OPT_WAVE` contains **zeros** in unextracted pixels — `build_spec` masks these.
...
def synthetic_spec1d(tmp_path):
    ...
    UVES echelle extractions look like.
    """
...
Run: `/Users/tim/conda/envs/pypeit3.14/bin/python -m pytest tests/test_pypeit_io.py -v`
Expected: PASS, 12 tests (2 of them marked `devsuite`, which pass on this machine and skip elsewhere)
