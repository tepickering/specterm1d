...
def test_zero_wavelengths_are_masked(synthetic_spec1d):
    # Reproduces real UVES OPT_WAVE zero padding.
    spec = registry.load(synthetic_spec1d)[0].spec()
    assert spec.good[:5].sum() == 0
    assert spec.good[-5:].sum() == 0
    assert spec.good[10:-10].all()
...
def test_encode_sixel_run_length_encodes_long_runs():
    indexed = np.zeros((6, 200), dtype=np.uint8)
    out = encode_sixel(indexed, PALETTE)
    assert "!" in out
    assert len(out) < 200
...
def _rle(codes: np.ndarray) -> str:
    ...
    while i < n:
        ...
        run = j - i
        if run >= 4:
            out.append(f"!{run}{chars[i]}")
        else:
            out.append(chars[i] * run)
        i = j
    return "".join(out)
