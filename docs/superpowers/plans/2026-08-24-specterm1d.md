...
def test_window_submode_x_prompts_for_a_range():
    session, _ = make_session()
    press(session, "w", "x")
    for ch in "5100 5300":
        session.handle(Key("char", ch))
    session.handle(Key("enter"))
    assert session.view.xlim == pytest.approx((5100.0, 5300.0))


def test_window_submode_a_autoscales():
    session, _ = make_session()
    session.view.xlim = (5400.0, 5500.0)
    press(session, "w", "a")
    assert session.view.xlim[0] == pytest.approx(5000.0)
...
# ---- w: gtools window submode ---------------------------------------
#
# The full gtools keymap is to be lifted from the IRAF gtools help file.
# These are the keys that manipulate the plot window; gtools keys that
# concern features this tool does not have report not-implemented, matching
# how the deferred top-level keys behave.

_WINDOW_OPTIONS = {
    "a": "autoscale both axes",
    "x": "set the x range",
    "y": "set the y range",
    "e": "expand between two cursors",
    "z": "zoom about the cursor",
    "p": "pan to centre the cursor",
    "q": "cancel",
}
...
def window(session):
    def chosen(sess, char):
        if char == "a":
            sess.view.reset_limits()
            sess.message("autoscaled")
        elif char == "x":
            def set_x(s, text):
                pair = _parse_pair(text or "")
                if pair is None:
                    s.message("expected two numbers, e.g. 5100 5300")
                    return
                s.view.xlim = (min(pair), max(pair))
                s.view.rescale_y()
                s.message(f"x range {s.view.xlim[0]:.6g} - {s.view.xlim[1]:.6g}")
            sess.await_line("x range (lo hi): ", set_x)
        elif char == "y":
            def set_y(s, text):
                pair = _parse_pair(text or "")
                if pair is None:
                    s.message("expected two numbers, e.g. 0 1.5")
                    return
                s.view.ylim = (min(pair), max(pair))
                s.message(f"y range {s.view.ylim[0]:.6g} - {s.view.ylim[1]:.6g}")
            sess.await_line("y range (lo hi): ", set_y)
        elif char == "e":
            expand(sess)
        elif char == "z":
            zoom(sess)
        elif char == "p":
            lo, hi = sess.view.xlim
            half = (hi - lo) / 2
            centre = sess.view.cursor_x
            sess.view.xlim = (float(centre - half), float(centre + half))
            sess.view.rescale_y()
            sess.message("panned")
        elif char == "q":
            sess.message("cancelled")
        else:
            sess.message(f"window: {char!r} is not a window key")

    session.await_key("window", chosen, _WINDOW_OPTIONS)
```
...
def test_e_reports_and_logs_an_equivalent_width():
    ...
    assert any(line.strip().startswith("5") for line in session.log.lines)
...
def test_e_takes_the_continuum_from_the_cursor_y():
    ...
    # make_session's flux is 1.0, so a continuum at 2.0 gives eqw != 0.
    assert "0.0000" not in session.last_message
...
**Still to be read during implementation:**

- The gtools help file, for the `w` window-submode keymap (Task 10).
