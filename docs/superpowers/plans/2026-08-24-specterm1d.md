...
Doing so caught two defects no assertion would have: the title was the file's
full absolute path, unreadable and clipped at both ends, and `right=0.99` cut
the final x tick label so `9000` rendered as `900`. `Session.title()` now uses
the basename and the figure margins are `left=0.09, right=0.975`.
...
- [ ] **Step 4: Look at a real pypeit render before declaring done**

Dump a frame from a pypeit `spec1d` file, not just the small test fixture, and
look at it. Doing so caught a defect the fixture could never show: pypeit
`spec1d` basenames run to about ninety characters, so with the object label
appended the title overran the axes and the label - the part that says which
order you are looking at - was clipped off the right.

`SpectrumPlot.fit_title` now elides the middle, favouring the tail so the
label survives. It measures the rendered width with matplotlib rather than
estimating from font metrics: a 0.62 em estimate put the cut-off within one
character of a real pypeit title and let it through still clipped.
...
## Post-implementation: defects found in manual use

The plan's automated verification could not catch either of these. Both came
from Tim actually running the tool.

**The halfblock render was illegible on macOS Terminal.** The backend asks for
one pixel per terminal column and two per row, so an 80x24 terminal gets an
80x44 pixel figure. Default 9pt tick labels are 12.5 px tall there - over a
quarter of the figure - so they overlapped into unreadable mush and the axis
labels were simply absent. Measured at 200x96 the x labels ran together as
`5670567556805685569056 95`.

Fixed by scaling the axis decoration to the pixel budget: `plot.chrome_for()`
returns one of three `Chrome` settings, and `SpectrumPlot` applies whichever
fits its current size. Above 500 px nothing changes. Between 150 and 500 px:
5 pt labels, four ticks per axis, no y label (horizontal pixels are scarcest),
tight margins. Below that: 4 pt, three ticks, no title or axis labels at all.
The 80x44 and 200x96 renders are legible after the change.

**`?` and `:show` did nothing visible.** `help_text()` was written in Task 9
and unit-tested, `display.help_page` set `showing_help` in Task 10, but no
task ever connected the flag to the render path - so both keys toggled a
variable nothing read. Task 17's check only verified that keys resolve to a
registered *handler*, not that the handler produces output.

Fixed by giving `Session` a full-screen text page, drawn as terminal text
rather than into the figure: at halfblock resolution rendered text is
illegible, while the terminal's own glyphs are crisp on every backend. It
pages with `<space>` and `b`, and any other key closes it - including `q`,
which must not quit the session while help is up.
...
### Defect 3: halfblock chrome is illegible at real terminal sizes

Tim ran it at 116x43 with SF Mono 14. That is a **116x82 pixel figure** - the
whole plot. The first fix (`chrome_for`, commit `000e61f`) scaled matplotlib's
own decoration down to fit, and took it from "useless and completely
illegible" to legible in isolation, but his verdict was "better in
Terminal.app, but still not quite usable".

Rendering at his real size showed why font tuning could not finish the job:

- `CHROME_TINY` clipped the top y tick label in half (`top=0.99`) and the last
  x label at the right edge (`right=0.995`). Both were ordinary bugs, fixed by
  reserving a little of each edge.
- The remaining problem was not tunable. A 4pt label at 100 dpi is 5.6 px tall
  and 3.3 px per digit, so a digit is drawn across roughly 3 columns and 3
  half-rows of half-block cells. The smear is a property of the pixel budget,
  not the typeface.

**Resolution: the terminal draws the chrome.** `Renderer.text_chrome` is True
only for halfblock. In that mode `Session.plot_rect()` reserves a left gutter
sized to the widest y label, one title row and two axis rows, and the figure
renders through `CHROME_BARE` - axes full bleed, no ticks, labels, title or
spines. `term/chrome.py` then paints spines, tick marks, tick labels, title,
legend and axis labels as terminal text.

Consequences worth recording:

- `plot.tick_values()` is a pure function of the limits, with no figure and no
  axis, because the chrome needs the numbers matplotlib would have drawn but
  has to place them in cells rather than pixels.
- Full bleed makes `ax.get_position()` the identity box, so the existing cell
  -> data cursor mapping keeps working unchanged through the shrunk rect.
  `SpectrumPlot.bare` is a property whose setter restyles immediately: as a
  plain attribute the axes position lagged the flag by one frame, and
  `on_mouse` before the first render was offset by the gutter width.
- At 116x43 the data region goes from ~96x67 px under the old margins to
  112x76 px, so the curve gains pixels while the labels become native glyphs.
- `--dump` and the non-halfblock backends still use matplotlib's chrome, which
  is why `chrome_for` and the `CHROME_TINY` margin fixes both remain.
